...
class RefTimestepEmbedSequential(nn.Sequential, TimestepBlock):
    """
    A sequential module that passes timestep embeddings to the children that
    support it as an extra input.
    """
...
class MainTimestepEmbedSequential(nn.Sequential, TimestepBlock):
    """
    A sequential module that passes timestep embeddings to the children that
    support it as an extra input.
    """
    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(
                ...
                x = layer(x, emb, num_video_frames, image_only_indicator)
            elif isinstance(module, SpatialVideoTransformerRef):
                x = layer(
                    x,
                    context,
                    time_context,
