...
class MemoryEfficientCrossAttentionRef(nn.Module):
    # https://github.com/MatthieuTPHR/diffusers/blob/d80b531ff8060ec1ea982b65a1b8df70f73aa67c/src/diffusers/models/attention.py#L223
    def __init__(
        self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, **kwargs
    ):
    ...
    def forward(
        ...
        if reference_context is not None:
            context = torch.cat([context, reference_context], dim=1)
        k = self.to_k(context)
        v = self.to_v(context)
...
class BasicTransformerBlockRef(nn.Module):
    ATTENTION_MODES = {
        "softmax": CrossAttention,  # vanilla attention
        "softmax-xformers": MemoryEfficientCrossAttentionRef,  # ampere
    }
    ...
    def forward(
        self, x, context=None, additional_tokens=None, n_times_crossframe_attn_in_self=0, reference_context=None
    ):
        kwargs = {"x": x}
        ...
        if self.checkpoint:
            # inputs = {"x": x, "context": context}
            return checkpoint(self._forward, x, context, additional_tokens, n_times_crossframe_attn_in_self, reference_context)
            # return checkpoint(self._forward, inputs, self.parameters(), self.checkpoint)
        else:
            return self._forward(**kwargs)

    def _forward(
        self, x, context, additional_tokens, n_times_crossframe_attn_in_self, reference_context,
    ):
        x = (
            self.attn1(
                self.norm1(x),
...
class SpatialTransformerRef(nn.Module):
    """
    Transformer block for image-like data.
    First, project the input (aka embedding)
    and reshape to b, t, d.
