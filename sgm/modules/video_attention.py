...
class SpatialVideoTransformerRef(SpatialTransformerRef):
    def __init__(
        self,
        in_channels,
        n_heads,
    ...
    def forward(
        ...
        if reference_context is not None:
            reference_context = self.norm(reference_context)
        if not self.use_linear:
            x = self.proj_in(x)
            if reference_context is not None:
                reference_context = self.proj_in(reference_context)
        x = rearrange(x, "b c h w -> b (h w) c")
        if reference_context is not None:
            reference_context = rearrange(reference_context, "b c h w -> b (h w) c")
        if self.use_linear:
            x = self.proj_in(x)
            if reference_context is not None:
                reference_context = self.proj_in(reference_context)
