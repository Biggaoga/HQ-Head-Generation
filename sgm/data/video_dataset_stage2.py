...
class VideoTrainDataset(Dataset):
    def __init__(self, base_folder='/data/yanghaibo/datas/OBJAVERSE-LVIS/images', width=1024, height=576, sample_frames=25, stage1_config="configs/inference-v01.yaml", stage1_ckpt="ckpts/first_stage.pt"):
        """
        Args:
    ...
    def config_stage1_model(self, stage1_config, stage1_ckpt):
        denoise_model = create_model(stage1_config).cpu()
    ...
    def __getitem__(self, idx):
        ...
        frames_256 = []
        for i in range(16):
            frames.append(f"{(16+i):04d}.png")
            frames_256.append(f"{16+i}.jpg")
        ...
        # start_idx = random.randint(0, len(self.elevations) - 1) * 16
        # start_idx = random.randint(0, len(frames) - 1)
        start_idx = random.choice([random.randint(0,4), random.randint(12,15)]) + 16  # 随机取正脸到侧脸的帧去做生成任务
        # start_idx = 16
        ...
        for frame_idx in range(start_idx, (range_id + 1) * 16):
            selected_frames.append(frames[frame_idx-16])
            selected_frames_256.append(frames_256[frame_idx-16])
        ...
        pixel_values_256 = torch.empty((self.sample_frames, self.channels, self.height, self.width))

        # Load and process each frame
        for i, (frame_name, frame_256) in enumerate(zip(selected_frames, selected_frames_256)):
            ...
            frame_256_path = os.path.join(folder_path, frame_256)
            img, mask = self.load_im(frame_256_path)
            # Resize the image and convert it to a tensor
            img_resized = img.resize((self.width, self.height))
            img_tensor = torch.from_numpy(np.array(img_resized)).float()
            ...
            if self.channels == 3:
                ...
                img_normalized = img_normalized.mean(
                    dim=2, keepdim=True)  # For grayscale images
            pixel_values_256[i] = img_normalized


        
        pixel_values = rearrange(pixel_values, 't c h w -> c t h w')
        pixel_values_256 = rearrange(pixel_values_256, 't c h w -> c t h w')
        
        caption = chosen_folder + "_" + str(start_idx)
        
        return {'video': pixel_values, 'video_stage1': pixel_values_256, 'elevation': elevation, 'caption': caption, "fps_id": 7, "motion_bucket_id": 127}
...
class SyncDreamerEvalData(Dataset):
    ...
    def get_data_for_index(self, index):
        ...
        elevation = 0
        return prepare_inputs(input_img_fn, elevation, self.image_size)
...
class VideoDataset(pl.LightningDataModule):
    ...
    def setup(self):
        self.train_dataset = VideoTrainDataset(self.base_folder, self.width, self.height, self.sample_frames, self.additional_args['stage1_config'], self.additional_args['stage1_ckpt'])
