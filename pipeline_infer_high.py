...
parser.add_argument('--denoise_config', type=str, default="configs/multiview/inference-high.yaml")
parser.add_argument('--denoise_checkpoint', type=str, default="/data/gyj/hi3d_256/outputs/logs/train-v04/2025-07-30T16-42-01_train-v04_00/checkpoints/trainstep_checkpoints/epoch=000111-step=000002999.ckpt/global_step3000/mp_rank_00_model_states.pt")
...
def denoising(frames, frames_256, aes, mv, elevation):
    with torch.no_grad():        
        ...
        batch['video_stage1']=frames_256.unsqueeze(0)
...
def video_pipeline(frames, frames_256, key, args):
    # seed = args['seed']
    ...
    for it in range(num_iter):
        ...
        with torch.no_grad():
            results = denoising(frames, frames_256, args['aes'], args['mv'], args['elevation'])
    ...
    output_videos_dir = os.path.join(output_videos_dir, 'high_stage')
...
def process(args, key='image'):
    ...
    frame_list_256 = []
    for i in range(16):
        img_path = os.path.join(args['stage1_image_path'], f"{(i+16):04d}.png")
        img = cv2.imread(img_path)
        frame_list_256.append(img)
    frame_list_256 = [cv2.resize(f, (args['input_resolution'][0], args['input_resolution'][1])) for f in frame_list_256]
    frame_list_256 = [cv2.cvtColor(f, cv2.COLOR_BGR2RGB) for f in frame_list_256]
    
    frame_list_256 = [to_tensor(f) for f in frame_list_256]
    frame_list_256 = [(f - 0.5) * 2.0 for f in frame_list_256]
    frames_256 = torch.stack(frame_list_256, 1)
    frames_256 = frames_256.cuda()


    models['denoising_model'].num_samples = args['clip_size']
    models['denoising_model'].image_size = args['input_resolution']
    
    video_pipeline(frames, frames_256, key, args)

image_path_list = [os.path.join(params.image_path, sub) for sub in os.listdir(params.image_path)]

for image_path in image_path_list:
    basename=os.path.basename(image_path)
    print(f"processing subject: {basename}")
    white_image_path = os.path.join(image_path, "temp_image", "white.png")
    stage1_image_path = os.path.join(image_path, "low_stage")


    # 3. first step , generate first image, and save in "args.output_dir/first_step/first.mp4"
    infer_config = {
            "image_path": white_image_path,
            "stage1_image_path": stage1_image_path,
            "clip_size": 16,
            "input_resolution": [
                512,
                512
