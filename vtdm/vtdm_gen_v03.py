...
from sgm.models.diffusion import DiffusionEngineRef
from sgm.util import log_txt_as_img, exists, instantiate_from_config, get_obj_from_str, default
from sgm.modules.diffusionmodules.wrappers import OPENAIUNETWRAPPER
from safetensors.torch import load_file as load_safetensors
from .model import load_state_dict

class VideoLDM(DiffusionEngineRef):
    ...
    def init_from_ckpt(
        ...
        for k, v in sd.items():
            if "model.diffusion_model" in k:
                new_k = k.replace("model.diffusion_model", "reference.diffusion_model")
                add_dict[new_k] = v
            if "attn2" in k:
                del_list.append(k)
            ...
            if "open_clip" in k:
                del_list.append(k)
            if "embedders.1.aesthetic_" in k:
                new_k = k.replace("embedders.1.aesthetic_", "embedders.0.aesthetic_")
                add_dict[new_k] = v
                del_list.append(k)
            if "embedders.3.encoder" in k:
                new_k = k.replace("embedders.3.encoder", "embedders.2.encoder")
                add_dict[new_k] = v
                del_list.append(k)
        for k in del_list:
            del sd[k]
    ...
    def add_custom_cond(self, batch, infer=False):
        ...
        ref_input = image.half()
        ref_input = ref_input.unsqueeze(2).repeat(1, 1, self.num_samples, 1, 1)
        ref_input = rearrange(ref_input, 'b c t h w -> (b t) c h w')
        ref_input = self.encode_first_stage(ref_input)
        batch['ref_input'] = ref_input
        ...
        frames_stage1 = batch['video_stage1']
        frames_stage1_reshape = rearrange(frames_stage1, 'b c t h w -> (b t) c h w')
        frames_stage1_embedding = self.encode_first_stage(frames_stage1_reshape.half())
        batch['frames_stage1_embedding'] = frames_stage1_embedding
    ...
    def forward(self, x, batch):
        loss = self.loss_fn(self.model,self.reference, self.denoiser, self.conditioner, x, batch)
        loss_mean = loss.mean()
        ...
        **kwargs,
        ...
        def denoiser(input, sigma, c):
            return self.denoiser(
                self.model, self.reference, input, sigma, c, torch.cat([batch['ref_input']]*2), **additional_model_inputs
            )
        ...
        if sample:
            with self.ema_scope("Plotting"):
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    randn = (randn + batch['frames_stage1_embedding']) / 2
